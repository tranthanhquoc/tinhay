...
if ( ! class_exists( 'Light_News_Recent_Post' ) ) :

     
    class Light_News_Recent_Post extends WP_Widget {
        /**
        ...
        public function __construct() {
            $tp_widget_recent_post = array(
                ...
                'description' => esc_html__( 'Retrive Recent posts.', 'legit-news' ),
            );
            parent::__construct( 'legit_news_recent_post', esc_html__( 'TP : Recent Posts', 'legit-news' ), $tp_widget_recent_post );
        }
                    ...
                    <a href="<?php the_permalink( $post->ID ); ?>">
                        <?php 
                            if ( has_post_thumbnail( $post->ID ) ) :
                                ...
                                echo '<img src="' . esc_url( get_template_directory_uri() .'/assets/uploads/no-featured-image-590x650.jpg' ) . '" alt="'. the_title_attribute( 'echo=0' ) .'">';
                            endif; 
                        ?>
                    </a>
        ...
        public function form( $instance ) {
            $tp_title      = isset( $instance['title'] ) ? ( $instance['title'] ) : esc_html__( 'Recent Posts', 'legit-news' );
            $tp_number     = isset( $instance['number'] ) ? absint( $instance['number'] ) : 3;
            $tp_category   = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
           ?>

           <p>
               <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'legit-news' ); ?></label>
           ...
           <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'legit-news' ); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'legit-news' ); ?></label>
                <select id="<?php echo esc_attr( $this->get_field_id('category') ); ?>" name="<?php echo $this->get_field_name('category'); ?>" class="widefat" style="width:100%;">
                    ...
                    <?php 
                    $categories = legit_news_category_choices();
        ...
        */
        public function update( $new_instance, $old_instance ) {
            ...
            $instance['category'] = legit_news_sanitize_single_category( $new_instance['category'] );
