 ...
 */
function matina_news_customize_register( $wp_customize ) {
	$wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
	$wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
	$wp_customize->get_setting( 'header_textcolor' )->transport = 'postMessage';

	$wp_customize->register_section_type( 'Matina_News_Section_Upsell' );
     ...
     * @since 1.0.2
     */
    $wp_customize->add_section( new Matina_News_Section_Upsell(
        $wp_customize,
            'matina_news_theme_upsell',
            array(
                'title'    	=> esc_html__( 'Matina Pro', 'matina-news' ),
                'pro_text' 	=> esc_html__( 'Buy Now', 'matina-news' ),
...
add_action( 'customize_register', 'matina_news_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function matina_news_customize_preview_js() {

	wp_enqueue_script( 'matina-news-customizer', get_template_directory_uri() . '/inc/customizer/assets/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'matina_news_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function matina_news_customize_backend_scripts() {
	...
	wp_enqueue_style( 'mt-extend-customizer', get_template_directory_uri() . '/inc/customizer/assets/css/mt-extend-customizer.min.css', array(), esc_attr( $matina_news_theme_version ) );

	wp_enqueue_script( 'mt-extend-customizer', get_template_directory_uri(). '/inc/customizer/assets/js/mt-extend-customizer.min.js', array('jquery'), esc_attr( $matina_news_theme_version ), true );
...
add_action( 'customize_controls_enqueue_scripts', 'matina_news_customize_backend_scripts', 10 );
...
$matina_news_sub_sections = array(
...
foreach ( $matina_news_sub_sections as $key => $value ) {
