...
wp.customize.controlConstructor['mt-range'] = wp.customize.Control.extend({
    ...
    ready: function() {
        ...
        var control = this,
            ...
            input_default,
            changeAction,
            mt_range_input_number_timeout;

        // Update the text value
        jQuery( 'input[type=range]' ).on( 'mousedown', function() {

            range           = jQuery( this );
            range_input     = range.parent().children( '.mt-range-input' );
        ...
        function mt_autocorrect_range_input_number( input_number, timeout ) {
            ...
            clearTimeout( mt_range_input_number_timeout );

            mt_range_input_number_timeout = setTimeout( function() {
        ...
        jQuery( 'input.mt-range-input' ).on( 'change keyup', function() {

            mt_autocorrect_range_input_number( jQuery( this ), 1000);

        } ).on( 'focusout', function() {

            mt_autocorrect_range_input_number( jQuery( this ), 0);

        } );

        // Handle the reset button
        jQuery( '.mt-reset-slider' ).on('click', function() {
