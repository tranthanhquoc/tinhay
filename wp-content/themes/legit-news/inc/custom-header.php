 ...
 *
 * @uses legit_news_header_style()
 */
function legit_news_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'legit_news_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'legit_news_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'legit-news' ),
		),
	) );
}
add_action( 'after_setup_theme', 'legit_news_custom_header_setup' );

if ( ! function_exists( 'legit_news_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see legit_news_custom_header_setup().
	 */
	function legit_news_header_style() {
		...
		wp_add_inline_style( 'legit-news-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'legit_news_header_style', 10 );