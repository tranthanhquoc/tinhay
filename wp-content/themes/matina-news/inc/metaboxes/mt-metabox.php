...
add_action( 'add_meta_boxes', 'matina_news_metaboxes', 10, 2 );

if ( ! function_exists( 'matina_news_metaboxes' ) ) :
     ...
     * @since 1.0.0
     */
    function matina_news_metaboxes() {
    
        add_meta_box(
            'matina_news_post_sidebar',
            __( 'Post Meta Settings', 'matina-news' ),
            'matina_news_sidebar_meta_callback',
            'post',
            'normal',
            'default'
        );
...
if ( ! function_exists( 'matina_news_sidebar_meta_callback' ) ) :
     ...
     * @since 1.0.0
     */
    function matina_news_sidebar_meta_callback( $post ) {

        //global $post;

        // sidebar option
        $post_sidebar_option = matina_news_post_meta_sidebar_option();

        // layout option
        $post_layout_option = matina_news_post_meta_layout_option();
        ...
        wp_nonce_field( basename( __FILE__ ) , 'matina_news_post_meta_nonce' );
?>
        <div class="mt-meta-container mt-clearfix">

            <ul class="mt-meta-menu-wrapper">
                <li class="mt-meta-tab <?php if ( $post_identity_value == 'mt-metabox-info' ) { echo 'active'; } ?>" data-tab="mt-metabox-info"><span class="dashicons dashicons-info"></span><?php esc_html_e( 'Information', 'matina-news' ); ?></li>
                <li class="mt-meta-tab <?php if ( $post_identity_value == 'mt-metabox-sidebar' ) { echo 'active'; } ?>" data-tab="mt-metabox-sidebar"><span class="dashicons dashicons-exerpt-view"></span><?php esc_html_e( 'Sidebars', 'matina-news' ); ?></li>
                <li class="mt-meta-tab <?php if ( $post_identity_value == 'mt-metabox-layout' ) { echo 'active'; } ?>" data-tab="mt-metabox-layout"><span class="dashicons dashicons-layout"></span><?php esc_html_e( 'Layouts', 'matina-news' ); ?></li>
            </ul>
            ...
            <div class="mt-metabox-content-wrapper">
                ...
                <div class="mt-single-meta <?php if ( $post_identity_value == 'mt-metabox-info' ) { echo 'active'; } ?>" id="mt-metabox-info">
                    <div class="content-header">
                        <h4><?php esc_html_e( 'About Metabox Options', 'matina-news' ) ;?></h4>
                    </div><!-- .content-header -->
                    <div class="meta-options-wrap"><?php esc_html_e( 'In this section we have lots of features which make your post unique and completely different.', 'matina-news' ); ?></div><!-- .meta-options-wrap  -->
                </div><!-- #mt-metabox-info -->

                <!-- Sidebar tab content -->
                <div class="mt-single-meta" id="mt-metabox-sidebar">
                    <div class="content-header">
                        <h4><?php esc_html_e( 'Available Sidebars', 'matina-news' ) ;?></h4>
                        <span class="section-desc"><em><?php esc_html_e( 'Select sidebar from available options which replaced sidebar layout from customizer settings.', 'matina-news' ); ?></em></span>
                    </div><!-- .content-header -->
                ...
                <div class="mt-single-meta" id="mt-metabox-layout">
                    <div class="content-header">
                        <h4><?php esc_html_e( 'Available Layouts', 'matina-news' ) ;?></h4>
                        <span class="section-desc"><em><?php esc_html_e( 'Select post layout from available options which replaced post layout from customizer settings.', 'matina-news' ); ?></em></span>
                    </div><!-- .content-header -->
                    <div class="mt-meta-options-wrap">
                        <div class="buttonset">
...
if ( ! function_exists( 'matina_news_save_post_meta' ) ) :
     ...
     * @since 1.0.0
     */
    function matina_news_save_post_meta( $post_id ) {

        // Verify the nonce before proceeding.
        $matina_news_post_meta_nonce   = isset( $_POST['matina_news_post_meta_nonce'] ) ? $_POST['matina_news_post_meta_nonce'] : '';
        $matina_news_post_meta_nonce_action = basename( __FILE__ );

        //* Check if nonce is set...
        if ( ! isset( $matina_news_post_meta_nonce ) ) {
            return;
        }

        //* Check if nonce is valid...
        if ( ! wp_verify_nonce( $matina_news_post_meta_nonce, $matina_news_post_meta_nonce_action ) ) {
            return;
        }
...
add_action( 'save_post', 'matina_news_save_post_meta' );