...
if ( ! class_exists( 'Matina_News_Control_Repeater' ) ) {
     ...
     * @since 1.0.0
     */
    class Matina_News_Control_Repeater extends WP_Customize_Control {
        ...
        public $matina_news_news_box_label = '';

        public $matina_news_news_box_add_control = '';
        ...
        public function __construct( $manager, $id, $args = array(), $fields = array() ) {
            ...
            $this->fields = $fields;
            $this->matina_news_box_label         = $args['matina_news_box_label_text'] ;
            $this->matina_news_box_add_control   = $args['matina_news_box_add_control_text'];
            parent::__construct( $manager, $id, $args );
        }

        public function enqueue() {            
            wp_enqueue_style( 'matina-news-repeater-style', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.css', null );
            wp_enqueue_script( 'matina-news-repeater-script', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.js', array( 'jquery' ), false, true );
        }
        ...
        ?>
            ...
            <ul class="mt-repeater-field-control-wrap">
                <?php $this->matina_news_get_fields(); ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="mt-repeater-collector" value="" />
            <button type="button" class="button mt-repeater-add-control-field"><i class="fas fa-plus"></i><?php echo esc_html( $this->matina_news_box_add_control ); ?></button>
    ...
    <?php
        ...
        private function matina_news_get_fields() {
            $fields = $this->fields;
            $values = $this->value();
        ...
        ?>
                ...
                <h3 class="mt-repeater-field-title"><?php echo esc_html( $this->matina_news_box_label ); ?></h3>
                ...
                ?>
                    ...
                    <?php 
                        ...
                        <?php
                             ...
                             */
                            case 'icon':
                                $matina_news_font_awesome_icon_array = matina_news_font_awesome_icon_array();
                                ...
                                foreach ( $matina_news_font_awesome_icon_array as $matina_news_font_awesome_icon ) {
                                    $icon_class = $new_value == $matina_news_font_awesome_icon ? 'icon-active' : '';
                                    echo '<li class='.esc_attr( $icon_class ).'><i class="'.esc_attr( $matina_news_font_awesome_icon ).'"></i></li>';
                                }
                             ...
                             */
                            case 'social_icon':
                                $matina_news_font_awesome_social_icon_array = matina_news_font_awesome_social_icon_array();
                                echo '<div class="mt-repeater-selected-icon"><i class="'.esc_attr( $new_value ).'"></i><span><i class="fa fa-angle-down"></i></span></div><ul class="mt-repeater-icon-list mt-clearfix">';
                                foreach ( $matina_news_font_awesome_social_icon_array as $matina_news_font_awesome_icon ) {
                                    $icon_class = $new_value == $matina_news_font_awesome_icon ? 'icon-active' : '';
                                    echo '<li class='.esc_attr( $icon_class ).'><i class="'.esc_attr( $matina_news_font_awesome_icon ).'"></i></li>';
                                }
                                echo '</ul><input data-default="'.esc_attr( $default ).'" type="hidden" value="'.esc_attr( $new_value ).'" data-name="'.esc_attr( $key ).'"/>';
                             ...
                             */
                            case 'dropdown_pages':
                                $show_option_none = esc_html__( '&mdash; Select a page &mdash;', 'matina-news' );
                            ...
                            case 'upload':
                                $image_class = "";
                                $upload_btn_label = esc_html__( 'Select Image', 'matina-news' );
                                $remove_btn_label = esc_html__( 'Remove', 'matina-news' );
                                if ( $new_value ){ 
                                    $image_class = ' hidden';
                                }
                                echo '<div class="mt-fields-wrap"><div class="attachment-media-view"><div class="placeholder'. esc_attr( $image_class ).'">';
                                esc_html_e( 'No image selected', 'matina-news' );
            ...
            ?>
                    <div class="mt-clearfix mt-repeater-footer">
                        ...
                        <a class="mt-repeater-field-remove" href="#remove"><?php esc_html_e( 'Delete', 'matina-news' ) ?></a> |
                        <a class="mt-repeater-field-close" href="#close"><?php esc_html_e( 'Close', 'matina-news' ) ?></a>
                        </div>
