...
if ( ! class_exists( 'Matina_News_Control_Typography' ) ){
     ...
     * @since 1.0.0
     */
    class Matina_News_Control_Typography extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->l10n = wp_parse_args(
                $this->l10n,
                array(
                    'family'    => esc_html__( 'Font Family', 'matina-news' ),
                    'style'     => esc_html__( 'Font Weight/Style', 'matina-news' )
                )
            );
        }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script( 'matina-news-google-webfont', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/webfontloader.min.js', array( 'jquery' ) );

            wp_enqueue_style( 'matina-news-select2-style', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/select2.min.css', null );
            
            wp_enqueue_script( 'matina-news-select2-script', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/select2.min.js', array( 'jquery' ), '4.0.13', true );

            wp_enqueue_script( 'matina-news-typography-script', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/typography.js', array( 'jquery', 'matina-news-select2-script' ), false, true );

            wp_enqueue_script( 'matina-news-typo-ajax-script', get_template_directory_uri() . '/inc/customizer/custom-controls/typography/typo-ajax.js', array( 'jquery', 'matina-news-select2-script', 'jquery-ui-slider' ), false, true );
            
            wp_localize_script( 'matina-news-typo-ajax-script', 'ajax_script', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
        }
            ...
            <li class="typography-color">
                ...
                <# } #>
                    ...
                    <div class="customize-control-content">
                        <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'matina-news' ); ?>" {{{ data.typocolor.link }}} value="{{ data.typocolor.value }}"  />
                    </div>
            </li>
        <#  } #>
        ...
        function get_font_families() {
            ...
            $mt_google_fonts_file = apply_filters( 'matina_news_google_fonts_json_file', get_template_directory() . '/inc/customizer/custom-controls/typography/mt-google-fonts.json' );
            ...
            $matina_news_google_font = get_option( 'matina_news_google_font' );
            if ( empty( $matina_news_google_font ) || $google_fonts != $matina_news_google_font ) {
                update_option( 'matina_news_google_font', $google_fonts );
                $matina_news_google_font = get_option( 'matina_news_google_font' );
            }

            foreach ( $matina_news_google_font as $key => $value ) {
        ...
        public function get_font_weight_choices() {
            ...
            if ( $this->settings['family']->id ) {
                ...
                $variants_array = $matina_news_font_list[$get_font_family]['0'];
                

                if ( is_array( $variants_array ) ) {
                    $options_array = array();
                    foreach ( $variants_array  as $variants ) {
                        $options_array[$variants] = matina_news_convert_font_variants( $variants );
                    }
                    return $options_array;
                } else {
                    return array(
                      '400' => esc_html__( 'Normal 400', 'matina-news' ),
                      '700' => esc_html__( 'Bold 700', 'matina-news' ),
                    );
                }
            } else {
                return array(
