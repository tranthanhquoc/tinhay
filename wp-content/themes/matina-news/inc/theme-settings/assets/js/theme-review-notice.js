jQuery(document).ready(function($) {
    ...
    switch( action ) {
        case 'activate' :
            $( '#mt-theme-message .mt-get-started' ).on( 'click', function() {
                var _this = $( this );
                matina_news_do_plugin( 'activate_demo_importer_plugin', _this );
            });
            break;
        case 'install' :
            $( '#mt-theme-message .mt-get-started' ).on( 'click', function() {
                var _this = $( this );
                matina_news_do_plugin( 'install_demo_importer_plugin', _this );
            });
            break;
        case 'redirect' :
            $( '#mt-theme-message .mt-get-started' ).on( 'click', function() {
    ...
    function matina_news_do_plugin( ajax_action, _this ) {
        $.ajax({
            method : "POST",
            url : WpAjaxurl,
            data : ({
